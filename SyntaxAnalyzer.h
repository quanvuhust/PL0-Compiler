...
class SyntaxAnalyzer
{
    ...
    private:
        std::unordered_map<int, const char*> errorString = {{0, "Missing ]"}, {1, "Missing )"},
            {2, "Condition: missing logic operator"}, {3, "Missing operator assign \':=\'"},
            {4, "Missing factor."}, {5, "Missing keyword END"}, {6, "Missing keyword THEN"}, {7, "Missing keyword DO in while loop"},
            {8, "Missing IDENT after FOR"}, {9, "Missing keyword TO in for loop"},
            {10, "Missing keyword DO in for loop"}, {11, "Missing keyword PROGRAM"},
            {12, "Missing name of program"}, {13, "Missing semicolon \';\'"}, {14, "Missing DOT \'.\'"},
            {15, "Missing keyword BEGIN"}, {16, "Missing operator \'=\'"}, {17, "Missing number when assign a const"},
            ...
            {23, "Missing procedure name when call PROCEDURE."}, {24, "Invalid term."}
        };
};
